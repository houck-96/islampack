...
import { normalizeArabicSuper } from './normalize.js';
...
export function normalizeArabic(text) {
  return normalizeArabicSuper(text);
}
export { normalizeArabicSuper };
...
export class IslamEngine {
  ...
  /**
   * ترميز رقم الحديث إلى عدد صحيح قابل للتعبئة.
   * المصادر تستخدم أرقاماً مكررة عشرية (مثل 402.2 للمكرر) — الترميز n×100
   * يحفظها مميزة (402→40200، 402.2→40220، 402.02→40202) ضمن 20 بت
   * (أقصى قيمة مرمزة 756300 < 1048575).
   */
  static encodeHadithNumber(n) {
    if (typeof n !== 'number' || !Number.isFinite(n) || n <= 0 || n > 20000) {
      throw new TypeError(`رقم الحديث يجب أن يكون رقماً موجباً محدوداً: ${n}`);
    }
    const enc = Math.round(n * 100);
    if (enc <= 0 || enc > 0xFFFFF) {
      throw new RangeError(`رقم الحديث المرمّز خارج النطاق: ${n}`);
    }
    return enc;
  }

  static resolvePackPath(filename) {
    // يقبل المسارات المطلقة مباشرة دون تحويل
  ...
  static _parsePackBuffer(buf, sourceLabel = 'الحزمة') {
    ...
    try {
      ...
      throw new Error(`فشل تحليل محتوى ${sourceLabel}: JSON غير صالح بعد فك الضغط.`);
    }
    return data;
  }
  ...
  static load(packPath = 'islam-full.pack') {
    ...
    engine.data = IslamEngine._parsePackBuffer(buf, resolved);
    engine._buildIndexes();
    return engine;
  }

  // نسخة غير حاجبة لمسار Event Loop (للخوادم) — نفس التحقق الصارم تماماً
  static async loadAsync(packPath = 'islam-full.pack') {
    const resolved = IslamEngine.resolvePackPath(packPath);
    const buf = await fs.promises.readFile(resolved);
    return IslamEngine.fromBuffer(buf, resolved);
  }

  // بناء محرك من بفر خام في الذاكرة (يُستخدم من الخادم عند رفع الحزم)
  static fromBuffer(buf, sourceLabel = 'الذاكرة') {
    const engine = new IslamEngine(typeof sourceLabel === 'string' ? sourceLabel : 'memory');
    engine.data = IslamEngine._parsePackBuffer(Buffer.from(buf), sourceLabel);
    engine._buildIndexes();
    return engine;
  }
  ...
  _buildIndexes() {
    ...
    // (ترميز الأرقام المكررة مثل 402.2 عبر encodeHadithNumber لتمييزها عن 402)
    if (this.data.hadith) {
        ...
        if (Array.isArray(list)) {
          for (const h of list) {
            if (h.t && typeof h.n === 'number') {
              const key = IslamicMPHF.packHadithKey(bId, IslamEngine.encodeHadithNumber(h.n));
  ...
  get quran() {
    return {
      getAyah: (surah, ayah, riwayah = "hafs") => {
        ...
        let rId;
        if (typeof riwayah === 'number') {
          rId = Number.isInteger(riwayah) ? riwayah : -1;
        } else {
          rId = RIWAYAT.indexOf(String(riwayah).toLowerCase());
        }
        if (rId < 0 || rId > 4) {
      ...
      // استرجاع السورة كاملة مرتبة (مع تطبيق طبقة الفوارق عند طلب رواية غير حفص)
      getSurah: (surah, riwayah = "hafs") => {
        ...
        for (let a = 1; a <= meta.ayahs; a++) {
          out.push({ surah, ayah: a, text: this.quran.getAyah(surah, a, riwayah) });
        }
        return out;
      },
  ...
  get hadith() {
    return {
      get: (book, number) => {
        ...
        if (!Number.isInteger(bId) || bId < 0 || bId > 4) throw new TypeError(`كتاب الحديث غير معروف: المتاح هو ${HADITH_BOOKS.join(', ')}.`);
        const num = typeof number === 'string' && number.trim() !== '' ? Number(number) : number;
        const enc = IslamEngine.encodeHadithNumber(num);

        const key = IslamicMPHF.packHadithKey(bId, enc);
        return this.hadithIndex.get(key) || null;
      },

      count: (book) => {
        const bId = typeof book === 'number' ? book : HADITH_BOOKS.indexOf(String(book).toLowerCase());
        if (!Number.isInteger(bId) || bId < 0 || !this.data.hadith?.[bId]) return 0;
        return this.data.hadith[bId].length;
      },

      totalCount: () => {
        if (!this.data.hadith) return 0;
        return Object.values(this.data.hadith).reduce((acc, list) => acc + (Array.isArray(list) ? list.length : 0), 0);
      }
    };
  }
  ...
  get tafsir() {
    return {
      get: (book, surah, ayah) => {
        ...
        if (!Number.isInteger(bId) || bId < 0 || bId > 4) throw new TypeError(`كتاب التفسير غير معروف: المتاح هو ${TAFSIR_BOOKS.join(', ')}.`);
        if (typeof surah !== 'number' || !Number.isInteger(surah) || surah < 1 || surah > 114) {
          throw new TypeError(`رقم السورة غير صحيح: يجب أن يكون بين 1 و 114.`);
        }
        const meta = SURAHS_META[surah - 1];
      ...
      totalCount: () => {
        if (!this.data.tafsir) return 0;
        return Object.values(this.data.tafsir).reduce((acc, list) => acc + (Array.isArray(list) ? list.length : 0), 0);
      }
    };
  }

  // ملخص إحصائي موحد للحزمة المحملة
  getStats() {
    return {
      packPath: this.packPath,
      quranVerses: this.quranIndex.size,
      quranVariants: this.quranVariants.size,
      hadithTotal: this.hadith.totalCount(),
      tafsirTotal: this.tafsir.totalCount(),
      hasQuran: this.quranIndex.size > 0,
      hasHadith: this.hadith.totalCount() > 0,
      hasTafsir: this.tafsir.totalCount() > 0
    };
  }
}